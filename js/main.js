...
function getContentTab(url,selector){
	...
	var loading = '<div class="a-center"><img src="//theme.hstatic.net/200000219361/1000665769/14/rolling.svg?v=34" alt="loading"/></div>';
	$.ajax({
		type: 'GET',
		url: url,
		beforeSend: function() {
...
$(document).ready(function(){
	...
	var img_array = $(".article-content p img");
	console.log(img_array);
	var img_array_alt = [];
	console.log(img_array_alt);
	if (img_array.length >= 1) {
		for (var i=0; i<img_array.length; i++) {
			if(img_array[i]['alt']=='áº£nh cáº§n chĂ¨n'){
				img_array_alt.push(img_array[i]['src']);
			}
		}
		console.log(img_array_alt);
		if (img_array_alt.length >= 1) {
			for (var j=0; j<img_array_alt.length; j++) {
				
				$('.wrapDuAn .listimage_article').append('<div class="item"><a href="'+img_array_alt[j]+'" data-toggle="lightbox" data-gallery="example-gallery"><i class="fa fa-search-plus"></i></a><img src="'+img_array_alt[j]+'" alt="image"/></div');
			}
